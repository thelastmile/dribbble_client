...
function pregame(name, path) {
  ...
  return function() {
    ...
    (function gameon(options, callback) {
      console.log('https://api.dribbble.com/v1/' + adjustedPath + (options ? '?' + querystring.stringify(options) : ''));
      request('https://api.dribbble.com/v1/' + adjustedPath + (options ? '?' + querystring.stringify(options) : ''), function(err, res, body) {
        var paging = {};
        var pages, page;
...
[{
	...
	name: 'list',
	path: 'shots?list=:list=popular,everyone,debuts'
}, {
	name: 'player',
	path: 'players/:id'
}, {
