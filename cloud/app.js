...
var saved_images_directory = "dribbble_saved_photos";
  ...
  if (Array.isArray(json)) {
    ...
    for (var i = 0; i < shotResultsArray.length; i++) {
    	...
    	if (shotResultsArray[i].images && shotResultsArray[i].images.hidpi) {
    		saveFileToDirectory(shotResultsArray[i].images.hidpi, fileName, saved_images_directory);
    	// Only "normal" size image is available:
    	} else if (shotResultsArray[i].images) {
    		saveFileToDirectory(shotResultsArray[i].images.normal, fileName, saved_images_directory);
    	// Image is an attachment:
    	} else {
    		saveFileToDirectory(shotResultsArray[i].url, fileName, saved_images_directory);
    	}
    }
  } else {
  	...
  	if (json.images) {
    	...
    	saved_images_directory = saved_images_directory + "/username:" + json.user.username + "-userid:" + json.user.id;

  		if (json.images.hidpi) {
  			saveFileToDirectory(json.images.hidpi, fileName, saved_images_directory);
  		} else {
  			saveFileToDirectory(json.images.normal, fileName, saved_images_directory);
  		}
...
var fetchShotsByUser = function(userId, directory) {
	baseRequest("users/" + userId + "?access_token=" + dribbble_client_access_token, null, function(err, res, json) {
		saved_images_directory = saved_images_directory + "/username:" + json.username + "-userid:" + json.id;
...
var saveFileToDirectory = function(fileUrl, fileName, directory) {
	...
	var file = fs.createWriteStream(directory + "/" + fileName /*+  ".jpg"*/);
